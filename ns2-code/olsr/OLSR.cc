...
///
static void olsr_mac_failed_callback(Packet *p, void *arg) {
...
public:
	OLSRHeaderClass() :
		PacketHeaderClass("PacketHeader/OLSR", sizeof(OLSR_pkt)) {
...
///
int OLSR::command(int argc, const char* const * argv) {
	if (argc == 2) {
		// Starts all timers
		if (strcasecmp(argv[1], "start") == 0) {
			//wsn_hello_z_ = 1; //erl
			wsn_hello_counter_ = 0; //erl
			//wsn_tc_z_ = 1; //erl	
			wsn_tc_counter_ = 0; //erl

			// erl - start the OLSR control traffic in a 10 s window from now.
			//random::uniform()*10.0;
			hello_timer_.resched(0.0 + Random::uniform() * 10.0);
			tc_timer_.resched(0.0 + Random::uniform() * 10.0);
			mid_timer_.resched(0.0 + Random::uniform() * 10.0);
			//	printf("%f (%d): Started timers maybe at %f\n",CURRENT_TIME,ra_addr(),Random::uniform()*10.0);
		...
		// Prints neighbor set
			...
			if (logtarget_ != NULL) {
				...
				fprintf(stdout,
						"%f _%d_ If you want to print this neighbor set "
							"you must create a trace file in your tcl script",
						CURRENT_TIME, OLSR::node_id(ra_addr()));
			}
			return TCL_OK;
		...
		// Prints 2-hop neighbor set
			...
			if (logtarget_ != NULL) {
				sprintf(logtarget_->pt_->buffer(),
						"P %f _%d_ Neighbor2hop Set", CURRENT_TIME,
						OLSR::node_id(ra_addr()));
				logtarget_->pt_->dump();
				OLSR_printer::print_nb2hopset(logtarget_, nb2hopset());
			} else {
				fprintf(stdout,
						"%f _%d_ If you want to print this neighbor2hop set "
							"you must create a trace file in your tcl script",
						CURRENT_TIME, OLSR::node_id(ra_addr()));
			}
			return TCL_OK;
		...
		// Prints MPR selector set
			...
			if (logtarget_ != NULL) {
				sprintf(logtarget_->pt_->buffer(),
						"P %f _%d_ MPR Selector Set", CURRENT_TIME,
						OLSR::node_id(ra_addr()));
				logtarget_->pt_->dump();
				OLSR_printer::print_mprselset(logtarget_, mprselset());
			} else {
				fprintf(stdout,
						"%f _%d_ If you want to print this mpr selector set "
							"you must create a trace file in your tcl script",
						CURRENT_TIME, OLSR::node_id(ra_addr()));
			}
			return TCL_OK;
		...
		// Prints topology set
			...
			if (logtarget_ != NULL) {
				...
				fprintf(stdout,
						"%f _%d_ If you want to print this topology set "
							"you must create a trace file in your tcl script",
						CURRENT_TIME, OLSR::node_id(ra_addr()));
			}
			return TCL_OK;
...
///
void OLSR_HelloTimer::expire(Event* e) {
	agent_->send_hello();
...
/// \param e The event which has expired.
///
void OLSR_TcTimer::expire(Event* e) {
...
/// \param e The event which has expired.
///
void OLSR_MidTimer::expire(Event* e) {
...
/// \param e The event which has expired.
///
void OLSR_DupTupleTimer::expire(Event* e) {
...
/// \param e The event which has expired.
///
void OLSR_LinkTupleTimer::expire(Event* e) {
	double now = CURRENT_TIME;
	// erl - we need to check if the tuple has been deleted already by another timer..
...
/// \param e The event which has expired.
///
void OLSR_Nb2hopTupleTimer::expire(Event* e) {
	if (tuple_->time() < CURRENT_TIME) {
...
/// \param e The event which has expired.
///
void OLSR_MprSelTupleTimer::expire(Event* e) {
	if (tuple_->time() < CURRENT_TIME) {
		agent_->rm_mprsel_tuple(tuple_);

		// erl - restart the wsn_counters also.
		agent_->reset_wsn_counters();
		//printf("%f (%d) Reset counters in OLSR_MprSelTupleTimer::expire\n",CURRENT_TIME, agent_->ra_addr());
...
/// \param e The event which has expired.
///
void OLSR_TopologyTupleTimer::expire(Event* e) {
	if (tuple_->time() < CURRENT_TIME) {

		//if (agent_->ra_addr()==13&&CURRENT_TIME>100.0){
		//	printf("%f (%d): Will now remove the tuple dest-last %d %d\n", CURRENT_TIME, agent_->ra_addr(), tuple_->dest_addr_, tuple_->last_addr_);
		//}
...
/// \param e The event which has expired.
///
void OLSR_IfaceAssocTupleTimer::expire(Event* e) {
	if (tuple_->time() < CURRENT_TIME) {
...
/// \param e The event which has expired.
///
void OLSR_MsgTimer::expire(Event* e) {
	agent_->send_pkt();
	delete this;
}
...
OLSR::OLSR(nsaddr_t id) :
	Agent(PT_OLSR), hello_timer_(this), tc_timer_(this), mid_timer_(this) {
	...
	ansn_ = OLSR_MAX_SEQ_NUM;
		...
		//wsn_hello_z_ = 1; //erl
			...
			wsn_tc_ttl_timeout_ = 0; //erl
			prev_wsn_tc_ttl_timeout_ = 0; //erl
			ttl_incr_ = 0;
			
			
			
	wsn_last_sent_soltc_ = -10; // erl
	bind("wsn_z_base_", &wsn_z_base_); // erl
	bind("wsn_z_incr_", &wsn_z_incr_); // erl
	
	bind_bool("immediatetrans_", &immediatetrans_);
	bind_bool("tcsolicit_", &tcsolicit_);
	bind_bool("reversetc_", &reversetc_);
	bind("vtimeweight_", &vtimeweight_);
	bind_bool("tcttl_", &tcttl_);
	bind("force_vttc_max_", &force_vttc_max_);
	

	//KNOWROUTETOSINK = false;// erl temp
}
...
///
void OLSR::recv(Packet* p, Handler* h) {
...
void OLSR::recv_olsr(Packet* p) {
	...
	for (int i = 0; i < op->count; i++) {
		...
		OLSR_dup_tuple* duplicated = state_.find_dup_tuple(msg.orig_addr(),
				msg.msg_seq_num());
		if (duplicated == NULL) {
			...
			else if (msg.msg_type() == OLSR_SOLTC_MSG)
				process_soltc(msg, ih->saddr());
			else if (msg.msg_type() == OLSR_MID_MSG)
				process_mid(msg, ih->saddr());
			else {
				debug(
						"%f: Node %d can not process OLSR packet because does not "
							"implement OLSR type (%x)\n", CURRENT_TIME,
						OLSR::node_id(ra_addr()), msg.msg_type());
			}
		} else {
			// If the message has been considered for forwarding, it should
			// not be retransmitted again
			for (addr_list_t::iterator it = duplicated->iface_list().begin(); it
					!= duplicated->iface_list().end(); it++) {
				if (*it == ra_addr()) {
					do_forwarding = false;
					break;
				}
...
void OLSR::mpr_computation() {
	...
	for (nb2hopset_t::iterator it = nb2hopset().begin(); it
			!= nb2hopset().end(); it++) {
		OLSR_nb2hop_tuple* nb2hop_tuple = *it;
		bool ok = true;
		OLSR_nb_tuple* nb_tuple = state_.find_sym_nb_tuple(
				nb2hop_tuple->nb_main_addr());
		if (nb_tuple == NULL)
			ok = false;
		else {
			nb_tuple = state_.find_nb_tuple(nb2hop_tuple->nb_main_addr(),
					OLSR_WILL_NEVER);
			if (nb_tuple != NULL)
				ok = false;
			else {
				nb_tuple
						= state_.find_sym_nb_tuple(nb2hop_tuple->nb2hop_addr());
				if (nb_tuple != NULL)
					ok = false;
			}
		}
	...
	for (nb2hopset_t::iterator it = N2.begin(); it != N2.end(); it++) {
		...
		if (!found) {
			...
			for (nb2hopset_t::iterator it2 = it + 1; it2 != N2.end(); it2++) {
				OLSR_nb2hop_tuple* nb2hop_tuple2 = *it2;
				if (nb2hop_tuple1->nb_main_addr()
						== nb2hop_tuple2->nb_main_addr()) {
		...
		for (std::set<nsaddr_t>::iterator it2 = deleted_addrs.begin(); it2
				!= deleted_addrs.end(); it2++) {
	...
	while (N2.begin() != N2.end()) {
		...
		for (set<int>::iterator it = rs.begin(); it != rs.end(); it++) {
			int r = *it;
			if (r > 0) {
				for (std::vector<OLSR_nb_tuple*>::iterator it2 =
						reachability[r].begin(); it2 != reachability[r].end(); it2++) {
		...
		if (max != NULL) {
			...
			for (nb2hopset_t::iterator it = N2.begin(); it != N2.end(); it++) {
				OLSR_nb2hop_tuple* nb2hop_tuple = *it;
				std::set<nsaddr_t>::iterator it2 = nb2hop_addrs.find(
...
void OLSR::rtable_computation() {
	...
	for (nbset_t::iterator it = nbset().begin(); it != nbset().end(); it++) {
		...
		if (nb_tuple->status() == OLSR_STATUS_SYM) {
			...
			for (linkset_t::iterator it2 = linkset().begin(); it2
					!= linkset().end(); it2++) {
				OLSR_link_tuple* link_tuple = *it2;
				if (get_main_addr(link_tuple->nb_iface_addr())
						== nb_tuple->nb_main_addr() && link_tuple->time()
						>= CURRENT_TIME) {
					lt = link_tuple;

					rtable_.add_entry(link_tuple->nb_iface_addr(),
							link_tuple->nb_iface_addr(),
							link_tuple->local_iface_addr(), 1);
	...
	for (nb2hopset_t::iterator it = nb2hopset().begin(); it
		...
		if (ok) {
			...
			assert(entry != NULL); // erl: this has failed in some cases in the past, due to wrong timer settings.
			rtable_.add_entry(nb2hop_tuple->nb2hop_addr(), entry->next_addr(),
					entry->iface_addr(), 2);
		}
	}
	...
	for (u_int32_t h = 2;; h++) {
		...
		for (topologyset_t::iterator it = topologyset().begin(); it
				!= topologyset().end(); it++) {
		...
		// erl - reverse topology tuplet link

		if (reversetc_ == true){
			for (topologyset_t::iterator it = topologyset().begin(); it
					!= topologyset().end(); it++) {
				OLSR_topology_tuple* topology_tuple = *it;
				OLSR_rt_entry* entry1 = rtable_.lookup(topology_tuple->last_addr());
				OLSR_rt_entry* entry2 = rtable_.lookup(topology_tuple->dest_addr());
				if (entry1 == NULL && entry2 != NULL && entry2->dist() == h) {
					rtable_.add_entry(topology_tuple->last_addr(),
							entry2->next_addr(), entry2->iface_addr(), h + 1);
					added = true;
				}
			}
		...
		for (ifaceassocset_t::iterator it = ifaceassocset().begin(); it
				!= ifaceassocset().end(); it++) {
			...
			if (entry1 != NULL && entry2 == NULL) {
				rtable_.add_entry(tuple->iface_addr(), entry1->next_addr(),
						entry1->iface_addr(), entry1->dist());
				added = true;
			}
		}
	...
	// erl - a check for the existence of a route to the sink (39). Make noise if we got it after not having it.

	/*
	OLSR_rt_entry* entry = rtable_.lookup(39);
	if (entry != NULL && KNOWROUTETOSINK == false) {
		//printf("%f (%d): Just got route to sink.\n", CURRENT_TIME, ra_addr());
		KNOWROUTETOSINK = true;
	} else if (entry == NULL && KNOWROUTETOSINK == true) {
		//printf("%f (%d): Just lost route to sink.\n", CURRENT_TIME, ra_addr());
		KNOWROUTETOSINK = false;
	}
	*/
...
///
void OLSR::process_hello(OLSR_msg& msg, nsaddr_t receiver_iface,
...
/// \param sender_iface the address of the interface where the message was sent from.
///
void OLSR::process_tc(OLSR_msg& msg, nsaddr_t sender_iface) {
	assert(msg.msg_type() == OLSR_TC_MSG);
	double now = CURRENT_TIME;
	OLSR_tc& tc = msg.tc();

	//if (ra_addr()==13&&msg.orig_addr_==3){
	//printf("%f (%d): Received TC from 3 with valid time %f\n",now, ra_addr(),emf_to_seconds(msg.vtime()));
	//}
	...
	// performed.
	OLSR_topology_tuple* topology_tuple = state_.find_newer_topology_tuple(
			msg.orig_addr(), tc.ansn());
	...
	for (int i = 0; i < tc.count; i++) {
		...
		OLSR_topology_tuple* topology_tuple = state_.find_topology_tuple(addr,
				msg.orig_addr());
		if (topology_tuple != NULL)
...
/// \param sender_iface the address of the interface where the message was sent from.
///
void OLSR::process_mid(OLSR_msg& msg, nsaddr_t sender_iface) {
	...
	for (int i = 0; i < mid.count; i++) {
		...
		for (ifaceassocset_t::iterator it = ifaceassocset().begin(); it
				!= ifaceassocset().end(); it++) {
			OLSR_iface_assoc_tuple* tuple = *it;
			if (tuple->iface_addr() == mid.iface_addr(i) && tuple->main_addr()
					== msg.orig_addr()) {
...
/**
 * Erl: solicitation for tc's to get to the destination.
 */
void OLSR::process_soltc(OLSR_msg& msg, nsaddr_t sender_iface) {
	assert(msg.msg_type() == OLSR_SOLTC_MSG);
	double now = CURRENT_TIME;
	OLSR_soltc& soltc = msg.soltc();
	...
	// We will not answer this message if we're not the addressed responding node.
	if (ra_addr() != soltc.resp_addr_)
		return;

	// 2. For the destination address listed in the SOLTC message
	//OLSR_rt_entry* entry = rtable_.lookup(ih->daddr());
	//if (entry == NULL) {
	// No, the destination is not in our routing table. Discard.
	//return;
	//} else {
	// Ok, now we have to collect the tc-messages that make up the path
	// to the destination...
	// All the entries from the tcrouting table are removed.
	tcrtable_.clear();

	// We start with the next hop, and then build outwards till we find the destination.
	// We take all TC-links that are iterated, and send them to the downstream neighbor.

	tcrtable_.add_entry(msg.orig_addr_, msg.orig_addr_, msg.orig_addr_, 1);
	for (u_int32_t h = 1;; h++) {
		bool added = false;
		...
		for (topologyset_t::iterator it = topologyset().begin(); it
				!= topologyset().end(); it++) {
			OLSR_topology_tuple* topology_tuple = *it;
			OLSR_rt_entry* entry1 = tcrtable_.lookup(
					topology_tuple->dest_addr());
			OLSR_rt_entry* entry2 = tcrtable_.lookup(
					topology_tuple->last_addr());
			if (entry1 == NULL && entry2 != NULL && entry2->dist() == h) {
				tcrtable_.add_entry(topology_tuple->dest_addr(),
						entry2->next_addr(), entry2->iface_addr(), h + 1);
				added = true;

				// Create TC-message and send it.
				send_onetclinkmessage(topology_tuple);

				if (entry2->dest_addr() == soltc.dst_addr_)
					return;
...
///
void OLSR::forward_default(Packet* p, OLSR_msg& msg, OLSR_dup_tuple* dup_tuple,
	...
	if (dup_tuple != NULL && dup_tuple->retransmitted()) {
		debug("%f: Node %d does not forward a message received"
			" from %d because it is duplicated\n", CURRENT_TIME, OLSR::node_id(
				ra_addr()), OLSR::node_id(dup_tuple->addr()));
		return;
	}
	...
	if (msg.ttl() > 1) {
		OLSR_mprsel_tuple* mprsel_tuple = state_.find_mprsel_tuple(
				get_main_addr(ih->saddr()));
	...
	// ...or create a new one
		...
		OLSR_DupTupleTimer* dup_timer = new OLSR_DupTupleTimer(this, new_dup);
...
///
void OLSR::forward_data(Packet* p) {
	struct hdr_cmn* ch = HDR_CMN(p);
	struct hdr_ip* ih = HDR_IP(p);
	...
	if (ch->direction() == hdr_cmn::UP && ((u_int32_t) ih->daddr()
		...
		if ((u_int32_t) ih->daddr() == IP_BROADCAST)
			...
			OLSR_rt_entry* entry = rtable_.lookup(ih->daddr());
			//printf("%f (%d): will do my best to route the pkt to destination %d.\n",CURRENT_TIME, ra_addr(),ih->daddr());
			if (entry == NULL) {
				debug("%f: Node %d can not forward a packet destined to %d\n",
						CURRENT_TIME, OLSR::node_id(ra_addr()), OLSR::node_id(
								ih->daddr()));
				//if (ra_addr()==13){

				//}
				// TODO Add IF to exempt packets originating from the node from triggering tc solicitation.
				if (ra_addr() != ih->saddr()) {
					solicitTC(ch->prev_hop_, ih->daddr()); // erl: need to ask for route info from *upstream* node if we are new in this network.
				}
...
void OLSR::solicitTC(nsaddr_t resp, nsaddr_t dst) {
	bool STATE_NEW = true;
	if (STATE_NEW && tcsolicit_)
		send_soltc(resp, dst);
}

// erl, enable the mac layer to check up the newest next hop.
nsaddr_t OLSR::lookup(nsaddr_t a)
{
//printf("%f (%d) Looking up %d\n",CURRENT_TIME,ra_addr(),a);
...
void OLSR::send_pkt() {
	...
	for (int i = 0; i < num_pkts; i++) {
		...
		int j = 0;
		for (std::vector<OLSR_msg>::iterator it = msgs_.begin(); it
				!= msgs_.end(); it++) {
			if (j == OLSR_MAX_MSGS)
				break;
...
void OLSR::send_hello() {
	...
	double v_hello = get_v(hello_ival(), wsn_hello_counter_);	
	double vt_hello = get_vt(hello_ival(), wsn_hello_counter_);

				
	if (vt_hello>OLSR_MAX_TIMEENC){ 
		vt_hello = OLSR_MAX_TIMEENC;
		
		if (v_hello > OLSR_MAX_TIMEENC/3.0){
			v_hello = OLSR_MAX_TIMEENC/3.0;
		}
	}
			
	msg.vtime() = OLSR::seconds_to_emf(vt_hello);


if (v_hello*3>vt_hello) printf("Warning! The three next hello messages are not covered by the timeout interval.\n");

	//if(ra_addr()==3)
	//printf("%f (%d): Sending Hello with vtime %f\n",CURRENT_TIME, ra_addr(),OLSR_NEIGHB_HOLD_TIME * (wsn_hello_z_
	//	+ wsn_incr_factor_z_));
	...
	msg.hello().htime() = OLSR::seconds_to_emf(v_hello); // erl
	...
	for (linkset_t::iterator it = linkset().begin(); it != linkset().end(); it++) {
		...
		if (link_tuple->local_iface_addr() == ra_addr() && link_tuple->time()
				>= now) {
			...
			if (state_.find_mpr_addr(get_main_addr(link_tuple->nb_iface_addr())))
				...
				bool ok = false;
				for (nbset_t::iterator nb_it = nbset().begin(); nb_it
						!= nbset().end(); nb_it++) {
...
// Max mantissa-exp is 3968 with a C of 1/16s
wsn_hello_counter_++;
}
...
void OLSR::send_tc() {
	...
	double v_tc  = get_v (tc_ival(), wsn_tc_counter_);
	double vt_tc = get_vt(tc_ival(), wsn_tc_counter_);

	//Note - forcing the validity time to MAX for ALL TC messages.
if (force_vttc_max_)
	vt_tc = OLSR_MAX_TIMEENC;	

	if (vt_tc>=OLSR_MAX_TIMEENC){ 
		vt_tc = OLSR_MAX_TIMEENC;
		
		if (v_tc > OLSR_MAX_TIMEENC/3.0){
			v_tc = OLSR_MAX_TIMEENC/3.0;
		}
		
		//printf("%f %d: Reached max TC timeout. Setting v %f and vt %f\n",CURRENT_TIME,ra_addr(),v_tc,vt_tc);
	}
	else
	{
		//printf("%f %d: Sat v_tc %f and TC timeout to %f\n",CURRENT_TIME,ra_addr(),v_tc,vt_tc);
	}

	if (v_tc*3 > vt_tc) printf("Warning! The three next tc messages are not covered by the timeout interval.\n");
				
	msg.vtime() = OLSR::seconds_to_emf(vt_tc); 
	msg.orig_addr() = ra_addr();
	
	// erl - set ttl lower than 255 if just restarted. Must at least make sure 
	// the next TC to reach the whole network does this before the old one times
	// out.
	
	double wsn_tc_ttl_estpropdelay_ = 6.0;

	
	double v_next_tc  = get_v (tc_ival(), wsn_tc_counter_+1);
	double vt_next_tc = get_vt(tc_ival(), wsn_tc_counter_+1);
	
	// Note - forcing the validity time to MAX for ALL TC messages
if (force_vttc_max_)
	vt_next_tc = OLSR_MAX_TIMEENC;

	if (vt_next_tc>=OLSR_MAX_TIMEENC){ 
		vt_next_tc = OLSR_MAX_TIMEENC;
		
		if (v_next_tc > OLSR_MAX_TIMEENC/3.0){
			v_next_tc = OLSR_MAX_TIMEENC/3.0;
		}
	}


	double wsn_tc_ttl_pktbuf_ = v_tc + wsn_tc_ttl_estpropdelay_; // En pakkes buffer	
	//double wsn_tc_ttl_pktbuf_ = v_tc + v_next_tc + wsn_tc_ttl_estpropdelay_; // To pakkers buffer
	if (wsn_tc_ttl_pktbuf_+CURRENT_TIME > wsn_tc_ttl_timeout_){
		msg.ttl() = 255;
		ttl_incr_=0;
		//printf("%f %d: reset TC TTL to %d\n",CURRENT_TIME,ra_addr(),msg.ttl());	
		
		}
	else{ 
		//msg.ttl() = 3 + ttl_incr_++;
		msg.ttl() = 3 + wsn_tc_counter_;		
		if (ttl_incr_>252) ttl_incr_=0;
	}

	// ErL We disregard the ttl calc if the tcttl is disabled.
	if (!tcttl_)	{
		msg.ttl() = 255;
	}
	...
	*/
	...
	for (mprselset_t::iterator it = mprselset().begin(); it
			!= mprselset().end(); it++) {
	...
	if (vt_tc + CURRENT_TIME > wsn_tc_ttl_timeout_ && msg.ttl()==255){
		prev_wsn_tc_ttl_timeout_ = wsn_tc_ttl_timeout_;
//		wsn_tc_ttl_timeout_ = vt_tc + CURRENT_TIME;
		wsn_tc_ttl_timeout_ = get_vt(tc_ival(), wsn_tc_counter_) + CURRENT_TIME;
		
	//	printf("%f %d: prev %f, tout %f\n",CURRENT_TIME, ra_addr(), prev_wsn_tc_ttl_timeout_, wsn_tc_ttl_timeout_);
	}
...
/**
 * Creates a tc link message for the tuple defined.
 */
void OLSR::send_onetclinkmessage(OLSR_topology_tuple* t) {
	double now = CURRENT_TIME;
	OLSR_msg msg;
	msg.msg_type() = OLSR_TC_MSG;
	msg.vtime() = OLSR::seconds_to_emf(t->time_ - now);
	msg.orig_addr() = node_id(t->last_addr());
	msg.ttl() = 1;
	msg.hop_count() = 0;
	msg.msg_seq_num() = msg_seq();
	...
	assert(count >= 0 && count < OLSR_MAX_ADDRS);
	msg.tc().nb_main_addr(count) = node_id(t->dest_addr());
...
void OLSR::send_mid() {
	...
	double vt_tc = get_vt(tc_ival(), wsn_tc_counter_);
				...
				if (vt_tc>OLSR_MAX_TIMEENC){ 
					vt_tc = OLSR_MAX_TIMEENC;
				//	i_hello = OLSR_MAX_TIMEENC/3.0;
			// line below commented since we don't send MID.
			// 	wsn_tc_counter_--;
				}			
	
	
	msg.vtime() = OLSR::seconds_to_emf(vt_tc); // erl *2=2vt
	msg.orig_addr() = ra_addr();
	msg.ttl() = 255;
	msg.hop_count() = 0;
	msg.msg_seq_num() = msg_seq();
...
void OLSR::send_soltc(nsaddr_t resp, nsaddr_t dst) {
	double now = CURRENT_TIME;

	// Do not send a solicitation if less than 10 s since the last one.
	if (now - wsn_last_sent_soltc_ < 10.0)
		return;

	OLSR_msg msg;
	msg.msg_type() = OLSR_SOLTC_MSG;
	...
	msg.soltc().dst_addr_ = dst;
	msg.soltc().resp_addr_ = resp;

	msg.msg_size() = msg.size();

	// Erl: we enqueue this message to be sent NOW!
	printf("%f (%d): Sending solicitation to %d for destination %d\n",
			CURRENT_TIME, ra_addr(), resp, dst);
	enque_msg(msg, 0);
	wsn_last_sent_soltc_ = now;
}
...
void OLSR::link_sensing(OLSR_msg& msg, nsaddr_t receiver_iface,
	...
	for (int i = 0; i < hello.count; i++) {
		...
		if ((lt == OLSR_SYM_LINK && nt == OLSR_NOT_NEIGH) || (nt
				!= OLSR_SYM_NEIGH && nt != OLSR_MPR_NEIGH && nt
				!= OLSR_NOT_NEIGH))
			continue;
		...
		for (int j = 0; j < hello_msg.count; j++) {
			if (hello_msg.nb_iface_addr(j) == receiver_iface) {
				if (lt == OLSR_LOST_LINK) {
					...
					if ((lt == OLSR_SYM_LINK && link_tuple->sym_time() <= now)
							|| (lt == OLSR_ASYM_LINK && link_tuple->sym_time()
									> now)) {
						changed = true; //erl
					}

					link_tuple->sym_time() = now + OLSR::emf_to_seconds(
							msg.vtime());
					link_tuple->time() = link_tuple->sym_time()
	...
	if (created && link_tuple != NULL) {
		OLSR_LinkTupleTimer* link_timer = new OLSR_LinkTupleTimer(this,
				link_tuple);
		link_timer->resched(
				DELAY(MIN(link_tuple->time(), link_tuple->sym_time())));
		link_tuple->timer_ = link_timer;
	}
	// erl:
	else if (updated) {// erl 
		link_tuple->timer_->resched(
				DELAY(MIN(link_tuple->time(), link_tuple->sym_time())));
	}

	if (created || changed) {
		// erl if changed or new link, reset the exptimer and initiate HELLO/TC transmissions.
		// printf("%f (%d): Saw a link change and resetting the wsn timer.\n",now,ra_addr());

		// Resetting the HELLO and TC timers IF these have been incremented.

		reset_wsn_counters();
	//	printf("%f (%d) Reset counters in link_sensing bco %d\n", now,	ra_addr(), link_tuple->nb_iface_addr());
	}
}
...
void OLSR::populate_nb2hopset(OLSR_msg& msg) {
	...
	for (linkset_t::iterator it_lt = linkset().begin(); it_lt
			!= linkset().end(); it_lt++) {
						...
						if (nt == OLSR_SYM_NEIGH || nt == OLSR_MPR_NEIGH) {
							...
							if (nb2hop_addr != ra_addr()) {
								...
								OLSR_nb2hop_tuple* nb2hop_tuple =
										state_.find_nb2hop_tuple(
												msg.orig_addr(), nb2hop_addr);
							...
							//printf("%f (%d): The node %d is not my 2hop-neighbor. Resetting wsn_exptimer\n",now,ra_addr(),nb2hop_addr);
							reset_wsn_counters();
						//	printf("%f (%d) Reset counters in populate_nb2hopset\n",		now, ra_addr());
						}
					}
				}
			}
...
void OLSR::reset_wsn_counters() {
	if (wsn_hello_counter_ > 0) {
		hello_timer_.resched(hello_ival());
	}

	if (wsn_tc_counter_ > 0) {
		tc_timer_.resched(tc_ival());
	}
	
	//printf("%f %d: Reset intervals. Were: Hello(%1.0f) TC(%1.0f)\n",CURRENT_TIME, ra_addr(),wsn_hello_counter_,wsn_tc_counter_);	
	
	wsn_hello_counter_ = 0;
	wsn_tc_counter_ = 0;
			...
			for (int j = 0; j < hello_msg.count; j++) {
				if (hello_msg.nb_iface_addr(j) == ra_addr()) {
					...
					if (mprsel_tuple == NULL) {
						...
						mprsel_tuple->time() = now + OLSR::emf_to_seconds(
								msg.vtime());
						...
						// Erl with a new mpr selector, we reset the intervals.
						reset_wsn_counters();
						//printf("%f (%d) Reset counters in populate_mprselset\n",	now, ra_addr());

					} else {
						// Erl with a new or timed out mpr selector, we reset the intervals.
						if (mprsel_tuple->time() < now) {
							reset_wsn_counters();
						//	printf("%f (%d) Reset counters due to old mprsel_tuple in populate_mprselset\n", now, ra_addr());
						}
...
void OLSR::mac_failed(Packet* p) {
	...
	bool newlostlink = false; //erl

	debug("%f: Node %d MAC Layer detects a breakage on link to %d\n", now,
			OLSR::node_id(ra_addr()), OLSR::node_id(ch->next_hop()));

	if ((u_int32_t) ih->daddr() == IP_BROADCAST) {
		printf("Warning, the packet was a broadcast packet!!!\n");
	...
	if (link_tuple != NULL) {
		if (link_tuple->lost_time() < now && link_tuple->time() > now) { // erl
			newlostlink = true; // erl
		}

		link_tuple->lost_time() = now + OLSR_NEIGHB_HOLD_TIME; // always 6 s
		link_tuple->time() = now + OLSR_NEIGHB_HOLD_TIME; // always 6 s
		nb_loss(link_tuple);
	...
	if (newlostlink) {
		reset_wsn_counters();
		//printf("%f (%d) Reset counters in mac_failed\n", now, ra_addr());
		//printf("%f (%d): MAC callback! Rescheduling HELLO and TC.\n", now,
		//	ra_addr());
		if (immediatetrans_==true){
			hello_timer_.resched(0.0);
			tc_timer_.resched(0.0);
		}
		else {
			hello_timer_.resched(Random::uniform()*hello_ival());
			tc_timer_.resched(Random::uniform()*tc_ival());
		}
	}
}
...
void OLSR::set_hello_timer() {
	double v_hello =  get_v (hello_ival(), wsn_hello_counter_<1?0:wsn_hello_counter_-1);
	double vt_hello = get_vt(hello_ival(), wsn_hello_counter_<1?0:wsn_hello_counter_-1);
	...
	hello_timer_.resched((double) (v_hello - JITTER));
}
...
void OLSR::set_tc_timer() {
	double v_tc  = get_v (tc_ival(), wsn_tc_counter_<1?0:wsn_tc_counter_-1);
	double vt_tc = get_vt(tc_ival(), wsn_tc_counter_<1?0:wsn_tc_counter_-1);
	...
	tc_timer_.resched((double) (v_tc - JITTER));
}

///
/// \brief Schedule the timer used for sending MID messages.
///
void OLSR::set_mid_timer() {
	double v_mid = get_v(mid_ival(), wsn_tc_counter_<1?0:wsn_tc_counter_-1);
	double vt_mid = get_vt(mid_ival(), wsn_tc_counter_<1?0:wsn_tc_counter_-1);

	if (v_mid>OLSR_MAX_TIMEENC)
	{ 
		vt_mid = OLSR_MAX_TIMEENC;
		
		if (v_mid > OLSR_MAX_TIMEENC/3.0){
			v_mid = OLSR_MAX_TIMEENC/3.0;
		}
	}

	mid_timer_.resched((double) (v_mid - JITTER));
}
...
void OLSR::nb_loss(OLSR_link_tuple* tuple) {
	...
	if (state_.erase_mprsel_tuples(get_main_addr(tuple->nb_iface_addr()))
			== true) {
...
void OLSR::add_link_tuple(OLSR_link_tuple* tuple, u_int8_t willingness) {
	...
	debug("%f: Node %d adds link tuple: nb_addr = %d\n", now, OLSR::node_id(
			ra_addr()), OLSR::node_id(tuple->nb_iface_addr()));
...
void OLSR::updated_link_tuple(OLSR_link_tuple* tuple) {
	...
	OLSR_nb_tuple* nb_tuple = state_.find_nb_tuple(get_main_addr(
			tuple->nb_iface_addr()));
	if (nb_tuple != NULL) {
	...
	debug("%f: Node %d has updated link tuple: nb_addr = %d status = %s\n",
			now, OLSR::node_id(ra_addr()),
			OLSR::node_id(tuple->nb_iface_addr()), ((nb_tuple->status()
					== OLSR_STATUS_SYM) ? "sym" : "not_sym"));
}
...
void OLSR::add_nb_tuple(OLSR_nb_tuple* tuple) {
	debug("%f: Node %d adds neighbor tuple: nb_addr = %d status = %s\n",
			CURRENT_TIME, OLSR::node_id(ra_addr()), OLSR::node_id(
					tuple->nb_main_addr()), ((tuple->status()
					== OLSR_STATUS_SYM) ? "sym" : "not_sym"));
...
void OLSR::add_nb2hop_tuple(OLSR_nb2hop_tuple* tuple) {
	debug(
			"%f: Node %d adds 2-hop neighbor tuple: nb_addr = %d nb2hop_addr = %d\n",
			CURRENT_TIME, OLSR::node_id(ra_addr()), OLSR::node_id(
					tuple->nb_main_addr()), OLSR::node_id(tuple->nb2hop_addr()));
...
void OLSR::rm_nb2hop_tuple(OLSR_nb2hop_tuple* tuple) {
	debug(
			"%f: Node %d removes 2-hop neighbor tuple: nb_addr = %d nb2hop_addr = %d\n",
			CURRENT_TIME, OLSR::node_id(ra_addr()), OLSR::node_id(
					tuple->nb_main_addr()), OLSR::node_id(tuple->nb2hop_addr()));
...
void OLSR::add_topology_tuple(OLSR_topology_tuple* tuple) {
	debug(
			"%f: Node %d adds topology tuple: dest_addr = %d last_addr = %d seq = %d\n",
			CURRENT_TIME, OLSR::node_id(ra_addr()), OLSR::node_id(
					tuple->dest_addr()), OLSR::node_id(tuple->last_addr()),
			tuple->seq());
...
void OLSR::rm_topology_tuple(OLSR_topology_tuple* tuple) {
	debug(
			"%f: Node %d removes topology tuple: dest_addr = %d last_addr = %d seq = %d\n",
			CURRENT_TIME, OLSR::node_id(ra_addr()), OLSR::node_id(
					tuple->dest_addr()), OLSR::node_id(tuple->last_addr()),
			tuple->seq());
...
void OLSR::add_ifaceassoc_tuple(OLSR_iface_assoc_tuple* tuple) {
	debug(
			"%f: Node %d adds iface association tuple: main_addr = %d iface_addr = %d\n",
			CURRENT_TIME, OLSR::node_id(ra_addr()), OLSR::node_id(
					tuple->main_addr()), OLSR::node_id(tuple->iface_addr()));
...
void OLSR::rm_ifaceassoc_tuple(OLSR_iface_assoc_tuple* tuple) {
	debug(
			"%f: Node %d removes iface association tuple: main_addr = %d iface_addr = %d\n",
			CURRENT_TIME, OLSR::node_id(ra_addr()), OLSR::node_id(
					tuple->main_addr()), OLSR::node_id(tuple->iface_addr()));
...
bool OLSR::seq_num_bigger_than(u_int16_t s1, u_int16_t s2) {
	return (s1 > s2 && s1 - s2 <= OLSR_MAX_SEQ_NUM / 2) || (s2 > s1 && s2 - s1
			> OLSR_MAX_SEQ_NUM / 2);
}
...
int OLSR::degree(OLSR_nb_tuple* tuple) {
	...
	for (nb2hopset_t::iterator it = nb2hopset().begin(); it
		...
		if (nb2hop_tuple->nb_main_addr() == tuple->nb_main_addr()) {
			OLSR_nb_tuple* nb_tuple = state_.find_nb_tuple(
					nb2hop_tuple->nb_main_addr());
			if (nb_tuple == NULL)
				degree++;
		}
...
///
u_int8_t OLSR::seconds_to_emf(double seconds) {
...
///
double OLSR::emf_to_seconds(u_int8_t olsr_format) {
	// This implementation has been taken from unik-olsrd-0.4.5 (mantissa.c),
	// licensed under the GNU Public License (GPL)
	int a = olsr_format >> 4;
...
///
int OLSR::node_id(nsaddr_t addr) {
...
/// \brief erl - function to calculate the interval of the current control message
///
///\param vd the default interval
///\param alpha the base exponent for increment
///\param beta the linear increment
///\param counter the counter for number of consecutive packets that are sent of this message type without reset.
double OLSR::get_v(double vd, double counter)
{
double alpha = wsn_z_base_;
double beta = wsn_z_incr_;

double v = vd*(pow(alpha,counter)+beta*counter);

return v;
}

///\brief erl - function to calculate the timeout of the current control message

///\param vd the default interval
///\param alpha the base exponent for increment
///\param beta the linear increment
///\param counter the counter for number of consecutive packets that are sent of this message type without reset.
double OLSR::get_vt(double vd, double counter)
{
double alpha = wsn_z_base_;
double beta = wsn_z_incr_;
...
for (int i=0;i<3;i++)
vt+=vd*(pow(alpha,(counter+i))+beta*(counter+i));
