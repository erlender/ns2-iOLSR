...
/// Erl: %OLSR TC solicitation type.
#define OLSR_SOLTC_MSG		4
...
typedef struct OLSR_hello_msg {
	...
	inline u_int8_t& link_code() {
		return link_code_;
	}
	inline u_int8_t& reserved() {
		return reserved_;
	}
	inline u_int16_t& link_msg_size() {
		return link_msg_size_;
	}
	inline nsaddr_t& nb_iface_addr(int i) {
		return nb_iface_addrs_[i];
	}

	inline u_int32_t size() {
		return OLSR_HELLO_MSG_HDR_SIZE + count * ADDR_SIZE;
	}
...
typedef struct OLSR_tc {
	...
	inline nsaddr_t& nb_main_addr(int i) {
		return nb_main_addrs_[i];
	}

	inline u_int32_t size() {
		return OLSR_TC_HDR_SIZE + count * ADDR_SIZE;
	}
...
typedef struct OLSR_mid {
	...
	inline nsaddr_t& iface_addr(int i) {
		return iface_addrs_[i];
	}

	inline u_int32_t size() {
		return count * ADDR_SIZE;
	}

} OLSR_mid;

/// %OLSR SOLTC message.
typedef struct OLSR_soltc {

	/// Destination address TCs are solicited for.
	nsaddr_t dst_addr_;

	/// Respondent address. The node that is expected to respond to this solicitation.
	nsaddr_t resp_addr_;

	// Only the destination address is included in this tc solicitation
	// message, thus 1*ADDR_SIZE.
	inline u_int32_t size() {
		return 2 * ADDR_SIZE;
	}
...
typedef struct OLSR_msg {
	...
	inline OLSR_soltc& soltc() {
		return msg_body_.soltc_;
	}
	...
	inline u_int32_t size() {
		...
		if (msg_type() == OLSR_HELLO_MSG)
			...
			sz += mid().size();
		else if (msg_type() == OLSR_SOLTC_MSG)
			sz += soltc().size();
		return sz;
	}
...
typedef struct OLSR_pkt {
	...
	inline u_int16_t& pkt_seq_num() {
		return pkt_seq_num_;
	}
	inline OLSR_msg& msg(int i) {
		return pkt_body_[i];
	}
