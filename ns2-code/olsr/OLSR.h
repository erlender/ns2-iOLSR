...
/// erl
#define OLSR_MAX_TIMEENC 3968

/********** Intervals **********/

/// HELLO messages emission interval.
#define OLSR_HELLO_INTERVAL	hello_ival_

/// TC messages emission interval.
#define OLSR_TC_INTERVAL	tc_ival_

/// MID messages emission interval.
#define OLSR_MID_INTERVAL	tc_ival_
...
///
#define OLSR_REFRESH_INTERVAL	hello_ival_
...
/// Dup holding time.
#define OLSR_DUP_HOLD_TIME	15*OLSR_HELLO_INTERVAL
/// MID holding time.
...
class OLSR: public Agent {
	...
	double wsn_z_base_, wsn_z_incr_;
	
	double wsn_hello_counter_;
	double wsn_tc_counter_;
	
	//int wsn_incr_factor_z_;
	
	double wsn_tc_ttl_timeout_;
	double prev_wsn_tc_ttl_timeout_;
	double ttl_incr_;
	
	
	double wsn_last_sent_soltc_;
	
	

	//bool KNOWROUTETOSINK; // erl temp

	/// Routing table.
	OLSR_rtable rtable_;

	/**
	 * erl: the table needed to establish the tc-messages required for an incoming downstream node to reach the destination
	 */
	OLSR_rtable tcrtable_;
...
protected:
	...
	inline double& hello_ival() {
		return hello_ival_;
	}
	inline double& tc_ival() {
		return tc_ival_;
	}
	inline double& mid_ival() {
		return mid_ival_;
	}
	...
	inline linkset_t& linkset() {
		return state_.linkset();
	}
	inline mprset_t& mprset() {
		return state_.mprset();
	}
	inline mprselset_t& mprselset() {
		return state_.mprselset();
	}
	inline nbset_t& nbset() {
		return state_.nbset();
	}
	inline nb2hopset_t& nb2hopset() {
		return state_.nb2hopset();
	}
	inline topologyset_t& topologyset() {
		return state_.topologyset();
	}
	inline dupset_t& dupset() {
		return state_.dupset();
	}
	inline ifaceassocset_t& ifaceassocset() {
		return state_.ifaceassocset();
	}
	...
	// erl - added functions.
	void send_onetclinkmessage(OLSR_topology_tuple*);
	void process_soltc(OLSR_msg&, nsaddr_t);
	void solicitTC(nsaddr_t, nsaddr_t);
	void send_soltc(nsaddr_t, nsaddr_t);
	void reset_wsn_counters();
	bool is_mpr();
	double get_v(double, double);
	double get_vt(double, double);
