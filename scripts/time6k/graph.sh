...
#t="$t set yrange [0:*];";
if [ $i == "rtrtrans" ]; then 
t="$t set ylabel 'Transmissions/500s';";
...
t="$t set output 'pdf/time6k_iolsr_"$i".eps';"
...
col=4;
t="$t 'wsnolsr_lin/results/0-6000s_$i.coi' using $datacols title 'iOLSR lin' with $linestype lw $lw lc $col lt $col ps $ps axis x1y1,"
let col=col+1;
t="$t 'wsnolsr_exp2/results/0-6000s_$i.coi' using $datacols title 'iOLSR exp2' with $linestype lw $lw lc $col lt $col ps $ps axis x1y1,"
let col=col+3;
t="$t 'wsnolsr_exp3/results/0-6000s_$i.coi' using $datacols title 'iOLSR exp3' with $linestype lw $lw lc $col lt $col ps $ps axis x1y1"
let col=col+1;
